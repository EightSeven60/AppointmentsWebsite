 ...
 */
class CalendarSystemsDateTime extends Datetime {

  public static function valueCallback(&$element, $input, FormStateInterface $form_state) {
    $e_cal = _calendar_systems_factory();
    if (!$e_cal) {
    ...
    if ($input !== FALSE) {
      ...
      $e_cal = _calendar_systems_factory($timezone, 'en');
      ...
      try {
        ...
        $date_time_format = _calendar_systems_tmp_fix_string($date_time_format);
        $date_time_input = _calendar_systems_tmp_fix_string($date_time_input);
  ...
  public static function processDatetime(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if ($element['#date_date_element'] != 'none') {
      $e_cal = _calendar_systems_factory($element['#date_timezone'], 'en');
      //      if ($date instanceof DrupalDateTime && !$date->hasErrors()) {
      //
      //      }
      //      else {
    ...
    if (isset($element['date'])) {
      if ($e_cal) {
        ...
        $range = $element['#date_year_range'] ?:
          CalendarSystemsDateList:: calendarSystemsDatetimeRangeYears(
            $element['#date_year_range'], $date, 'gregorian');
        $e_cal->setDateLocale($range[0], 1, 1);
        $attr['min'] = $e_cal->getTimestamp();
        $e_cal->setDateLocale($range[1], 1, 1);
        $attr['max'] = $e_cal->getTimestamp();

        $attr['data-calendar-systems-calendar'] = $e_cal->getCalendarName();
        $attr['autocomplete'] = 'off';
        $attr['data-calendar-systems-format'] = $element['#date_date_format'];
      }
    }
  ...
  private static function xProcessDatetime(&$element, FormStateInterface $form_state, /** @noinspection PhpUnusedParameterInspection */ &$complete_form) {
    $format_settings = [];
    ...
    if ($element['#date_date_element'] != 'none') {
      ...
      if ($date instanceof DrupalDateTime && !$date->hasErrors()) {
        ...
        $range = CalendarSystemsDateList::calendarSystemsDatetimeRangeYears($element['#date_year_range'], DrupalDateTime::createFromTimestamp($date->getTimestamp()));
  ...
  public static function formatExample($format) {
    return (new CalendarSystemsDrupalDateTime())->format($format);
  }
